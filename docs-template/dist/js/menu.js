            ...
            if (hasChildren) {
                ...
                function showSubFloating() {
                    ...
                    if (!subFloatingMenu) {
                        ...
                        subFloatingMenu.classList.add('fixed', 'z-[2100]', 'opacity-0', 'pointer-events-none', 'transition-all', 'duration-200');
                        subFloatingMenu.style.display = 'block';
                        document.body.appendChild(subFloatingMenu);
                        subFloatingMenu.dataset.level = parseInt(container.closest('.floating-menu-bg')?.dataset.level || 0) + 1;
...
function renderMenu(items, container, depth = 0) {
    ...
    items.forEach(item => {
        ...
        if (item.header) {
            ...
            headerEl.className = 'px-4 pt-6 pb-2 text-[10px] font-bold uppercase tracking-widest text-muted/60 sidebar-hide transition-all duration-300';
            headerEl.innerHTML = `
            ...
            miniDivider.innerHTML = `<div class="h-[1px] w-full bg-slate-200 dark:bg-slate-800"></div>`;
        ...
        if (hasChildren) {
            ...
            function showFloating() {
                ...
                if (!floatingMenu) {
                    ...
                    floatingMenu.classList.add('fixed', 'z-[2000]', 'opacity-0', 'pointer-events-none', 'transition-all', 'duration-200');
        ...
        if (hasChildren || item.isMore) {
            ...
            function showFloating() {
                ...
                if (!floatingMenu) {
                    const optimalSide = getOptimalSide(link);
                    floatingMenu = buildFloatingMenu(item.children, optimalSide);
                    floatingMenu.classList.add('floating-menu-bg', 'top-nav-dropdown', 'fixed', 'z-[2000]', 'opacity-0', 'pointer-events-none', 'transition-all', 'duration-200');
                    floatingMenu.style.display = 'block';
                    document.body.appendChild(floatingMenu);
                    floatingMenu.dataset.level = 1;
