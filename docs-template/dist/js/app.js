...
function getVibeConfig(key) {
    ...
    return VIBE_CONFIG[key] !== undefined ? VIBE_CONFIG[key] : defaultSettings[key];
}
...
function setSidebarState(isMini) {
    ...
    if (miniState) {
        ...
        if (toggleIcon) toggleIcon.classList.replace('bx-chevron-left', 'bx-chevron-right');

        document.querySelectorAll('.sidebar-hide').forEach(el => el.classList.add('hidden'));
        document.querySelectorAll('.sidebar-mini-show').forEach(el => el.classList.remove('hidden'));
        document.querySelectorAll('.sidebar-nav-link').forEach(el => {
            el.classList.replace('justify-start', 'justify-center');
            el.style.paddingLeft = '';
        });
        document.querySelectorAll('.sidebar-icon').forEach(el => el.classList.remove('lg:mr-3'));

        document.querySelectorAll('.submenu-container').forEach(el => el.classList.remove('open'));
        document.querySelectorAll('.chevron-rotate').forEach(el => el.classList.remove('rotate'));

        if (logoutBtn) logoutBtn.classList.replace('justify-start', 'justify-center');
        ...
        if (toggleIcon) toggleIcon.classList.replace('bx-chevron-right', 'bx-chevron-left');

        document.querySelectorAll('.sidebar-hide').forEach(el => el.classList.remove('hidden'));
        document.querySelectorAll('.sidebar-mini-show').forEach(el => el.classList.add('hidden'));
        document.querySelectorAll('.sidebar-nav-link').forEach(el => {
        ...
        document.querySelectorAll('.sidebar-icon').forEach(el => el.classList.add('lg:mr-3'));

        if (logoutBtn) logoutBtn.classList.replace('justify-center', 'justify-start');
        ...
        if (!isMobile) localStorage.setItem('vibe-template.sidebar-theme', 'expanded');
    }
}
...
if (themeToggleBtn) {
    themeToggleBtn.addEventListener('click', () => {
        ...
        localStorage.setItem('vibe-template.color-theme', isDark ? 'dark' : 'light');
        updateThemeIcons();
...
const backgroundPresets = [
    { name: 'Neutral', light: '241, 245, 249', surface: '255, 255, 255', alt: '241, 245, 249', border: '226, 232, 240', muted: '100, 116, 139', sidebar: '71, 85, 105' },
    { name: 'Solid', light: '241, 245, 249', surface: '255, 255, 255', border: '226, 232, 240', sidebar: '71, 85, 105' },
    { name: 'Matching', light: '241, 245, 249', surface: '255, 255, 255', sidebar: '71, 85, 105' }
];

const darkPalettes = [
    { name: 'Classic Slate', body: '11, 17, 32', surface: '15, 23, 42', alt: '30, 41, 59', border: '51, 65, 85', muted: '148, 163, 184' },
    { name: 'Midnight Onyx', body: '10, 10, 10', surface: '23, 23, 23', alt: '38, 38, 38', border: '51, 51, 51', muted: '163, 163, 163' },
    { name: 'Charcoal Elegance', body: '28, 33, 40', surface: '34, 39, 46', alt: '45, 51, 59', border: '68, 76, 86', muted: '133, 142, 153' },
    { name: 'Oceanic Deep', body: '9, 14, 23', surface: '17, 24, 39', alt: '31, 41, 55', border: '55, 65, 81', muted: '110, 120, 133' },
];

const lightPalettes = [
    { name: 'Pure White', body: '255, 255, 255', surface: '255, 255, 255', alt: '248, 250, 252', border: '241, 245, 249', muted: '100, 116, 139' },
    { name: 'Soft Slate', body: '248, 250, 252', surface: '255, 255, 255', alt: '241, 245, 249', border: '226, 232, 240', muted: '100, 116, 139' },
    { name: 'Modern Gray', body: '243, 244, 246', surface: '255, 255, 255', alt: '229, 231, 235', border: '209, 213, 219', muted: '75, 85, 101' },
    { name: 'Oceanic Light', body: '240, 249, 255', surface: '255, 255, 255', alt: '224, 242, 254', border: '186, 230, 253', muted: '51, 65, 85' },
];
...
const canvasStyles = [
    ...
    },
    {
        ...
        mainClass: 'mb-4',
        canvasClass: 'p-4 rounded-[1.5rem] bg-surface/50 backdrop-blur-xl shadow-xl',
    ...
    {
        name: 'Floating',
        appClass: 'h-[calc(100vh-40px)] m-[20px] rounded-[1.5rem] border bg-body overflow-hidden shadow-2xl',
        mainClass: 'mr-4',
...
const zoomLevels = [
    ...
    { name: 'Standard', size: '14px', icon: 'bx-screenshot', desc: 'Default view' },
...
settingsPanel.className = 'bg-surface border-l shadow-2xl flex flex-col transition-all duration-300';
...
function updateBackgroundPreset(presetName) {
    ...
    const lightMatch = rgb.map(c => Math.round(c + (255 - c) * 0.82)).join(', ');
    const lightMatchAlt = rgb.map(c => Math.round(c + (255 - c) * 0.75)).join(', ');
    const lightMatchBorder = rgb.map(c => Math.round(c + (255 - c) * 0.6)).join(', ');
    const darkMatchSurface = rgb.map(c => Math.round(5 + c * 0.12)).join(', ');
    const darkMatchAlt = rgb.map(c => Math.round(15 + c * 0.20)).join(', ');
    const darkMatchBorder = rgb.map(c => Math.round(20 + c * 0.25)).join(', ');
    const darkMatchBody = rgb.map(c => Math.round(2 + c * 0.04)).join(', ');

    const presetData = backgroundPresets.find(p => p.name === presetName) || backgroundPresets[0];
    ...
    if (presetName === 'Solid') {
        const darkSolidSidebar = rgb.map(c => Math.round(15 + c * 0.25)).join(', ');

        document.documentElement.style.setProperty('--sidebar-bg', isDark ? darkSolidSidebar : currentAccent);
        document.documentElement.style.setProperty('--sidebar-text', '219, 234, 254');
        document.documentElement.style.setProperty('--sidebar-text-muted', '219, 234, 254, 0.65');
        document.documentElement.style.setProperty('--sidebar-link-active-bg', '255, 255, 255, 0.25');
        document.documentElement.style.setProperty('--sidebar-link-active-text', '255, 255, 255');
        document.documentElement.style.setProperty('--sidebar-border', isDark ? '51, 65, 85' : '255, 255, 255, 0.1');

        // Sidebar item active/hover — Solid mode uses translucent white
        document.documentElement.style.setProperty('--sidebar-item-text', 'rgba(219, 234, 254, 0.85)');
        document.documentElement.style.setProperty('--sidebar-item-active-bg', 'rgba(255, 255, 255, 0.25)');
        document.documentElement.style.setProperty('--sidebar-item-active-text', 'rgb(255, 255, 255)');
        document.documentElement.style.setProperty('--sidebar-item-hover-bg', 'rgba(255, 255, 255, 0.15)');
        document.documentElement.style.setProperty('--sidebar-item-hover-text', 'rgb(255, 255, 255)');

        // Floating menu solid sync
        document.documentElement.style.setProperty('--floating-bg', isDark ? darkSolidSidebar : currentAccent);
        document.documentElement.style.setProperty('--floating-text', '219, 234, 254');
        document.documentElement.style.setProperty('--floating-border', isDark ? '51, 65, 85' : '255, 255, 255, 0.1');
        document.documentElement.style.setProperty('--floating-hover-bg', 'rgba(255, 255, 255, 0.2)');
        document.documentElement.style.setProperty('--floating-hover-text', '#ffffff');
        document.documentElement.style.setProperty('--floating-active-bg', 'rgba(255, 255, 255, 0.25)');
        document.documentElement.style.setProperty('--floating-active-text', '#ffffff');
        ...
        document.documentElement.style.setProperty('--sidebar-bg', 'var(--color-bg-surface)');
        document.documentElement.style.setProperty('--sidebar-text', isDark ? '203, 213, 225' : '71, 85, 105');
        ...
        document.documentElement.style.setProperty('--floating-bg', 'var(--color-bg-surface)');
        document.documentElement.style.setProperty('--floating-text', isDark ? '203, 213, 225' : '71, 85, 105');
        ...
        if (elSidebar && canvasToken && canvasToken.sidebarClass) {
            elSidebar.classList.add(...canvasToken.sidebarClass.split(' ').filter(c => c));
        }
    }
    const paletteName = isDark ? getVibeConfig('dark-palette') : getVibeConfig('light-palette');
    ...
    if (presetName === 'Matching') {
        document.documentElement.style.setProperty('--color-bg-body', isDark ? darkMatchBody : lightMatch);
        document.documentElement.style.setProperty('--color-bg-surface', isDark ? darkMatchSurface : '255, 255, 255');
        document.documentElement.style.setProperty('--color-bg-alt', isDark ? darkMatchAlt : lightMatchAlt);
        document.documentElement.style.setProperty('--color-border', isDark ? darkMatchBorder : lightMatchBorder);
        document.documentElement.style.setProperty('--bg-gradient', isDark ? `rgb(${darkMatchBody})` : `linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)`);
    } else if (presetName === 'Solid') {
        // Blending logic for Solid: Neutral surfaces but with tinted borders and optimized body
        const solidBorder = isDark ? activePalette.border : rgb.map(c => Math.round(c + (255 - c) * 0.88)).join(', ');

        document.documentElement.style.setProperty('--color-bg-body', activePalette.body);
        document.documentElement.style.setProperty('--color-bg-surface', activePalette.surface);
        document.documentElement.style.setProperty('--color-bg-alt', activePalette.alt);
        document.documentElement.style.setProperty('--color-border', solidBorder);
        document.documentElement.style.setProperty('--bg-gradient', isDark
            ? `rgb(${activePalette.body.split(',').map(c => Math.max(0, parseInt(c) - 5)).join(',')})`
            : `linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)`);
    } else {
        document.documentElement.style.setProperty('--color-bg-body', activePalette.body);
        document.documentElement.style.setProperty('--color-bg-surface', activePalette.surface);
        document.documentElement.style.setProperty('--color-bg-alt', activePalette.alt);
        document.documentElement.style.setProperty('--color-border', activePalette.border);
        document.documentElement.style.setProperty('--bg-gradient', isDark ? `rgb(${activePalette.body})` : `linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)`);
    }

    // Sync Text Muted
    document.documentElement.style.setProperty('--color-text-muted', activePalette.muted);
    ...
    if (header) {
        const headerControls = header.querySelectorAll('#theme-toggle, #settings-toggle, .flex.p-1.rounded-lg[style*="rgb(var(--color-bg-alt))"]');
...
function updateCanvasStyle(styleName) {
    ...
    const styleToken = canvasStyles.find(s => s.name === styleName) || canvasStyles[0];
    const isMini = getVibeConfig('sidebar-theme') === 'mini';
    const navMode = getVibeConfig('nav-mode');
    const bgPreset = getVibeConfig('bg-preset');

    if (elApp) {
        elApp.className = 'flex transition-all duration-500 bg-body overflow-hidden';
        if (navMode === 'top') elApp.classList.add('layout-top-nav');
    }
    if (elCanvas) elCanvas.className = 'space-y-8 transition-all duration-500';
    if (elMain) elMain.className = 'flex-1 overflow-y-auto w-full space-y-8 transition-all duration-500';
    if (elSidebar) {
    ...
    if (elHeader) {
        elHeader.className = 'h-16 flex items-center justify-between px-8 sticky top-0 z-30 transition-all duration-500';
        // Note: header-solid will be re-applied by updateBackgroundPreset call at the end
    }

    if (elApp && styleToken.appClass) elApp.classList.add(...styleToken.appClass.split(' ').filter(c => c));
    if (elMain && styleToken.mainClass) elMain.classList.add(...styleToken.mainClass.split(' ').filter(c => c));
    if (elCanvas && styleToken.canvasClass) elCanvas.classList.add(...styleToken.canvasClass.split(' ').filter(c => c));
    if (elSidebar && styleToken.sidebarClass) elSidebar.classList.add(...styleToken.sidebarClass.split(' ').filter(c => c));
    if (elHeader && styleToken.headerClass) elHeader.classList.add(...styleToken.headerClass.split(' ').filter(c => c));
...
function updateNavMode(mode) {
    if (typeof clearAllFloatingMenus === 'function') clearAllFloatingMenus();
    const isMobile = window.innerWidth < 768; const activeMode = isMobile ? "sidebar" : mode;
...
function refreshActiveSwatches() {
    ...
    if (nameLabel) {
        const selectedAccent = accentColors.find(c => c.value === currentAccent) || accentColors[0];
    ...
    const activeClasses = isDark ? ['bg-white', 'text-slate-900', 'border-white'] : ['bg-primary-600', 'text-white', 'border-primary-600'];
    const allPossibleActiveClasses = ['bg-white', 'text-slate-900', 'border-white', 'bg-primary-600', 'text-white', 'border-primary-600'];
    ...
    if (darkNameLabel) {
        const selectedPalette = darkPalettes.find(p => p.name === currentPalette) || darkPalettes[0];
    ...
    if (lightNameLabel) {
        const selectedPalette = lightPalettes.find(p => p.name === currentLightPalette) || lightPalettes[1];
...
function initFloatingComponents() {
    if (typeof FloatingUIDOM === 'undefined') {
        console.warn('Floating UI library (FloatingUIDOM) not found. Tooltips and dropdowns will not be initialized.');
        return;
    }
    ...
    // --- Tooltip Management ---
    const tooltips = document.querySelectorAll('[data-vibe-tooltip], [data-tippy-content]');
    tooltips.forEach(ref => {
        const content = ref.getAttribute('data-vibe-tooltip') || ref.getAttribute('data-tippy-content');
        if (!content) return;

        const tooltip = document.createElement('div');
        tooltip.className = 'vibe-tooltip fixed bg-slate-800 text-white text-[11px] font-bold px-2 py-1.5 rounded-lg shadow-xl z-[2000] opacity-0 pointer-events-none transition-opacity duration-200';
...
window.vibeToast = {
    ...
    show({ title = 'Notification', message = '', type = 'info', position = 'bottom-right', duration = 5000 }) {
        ...
        if (position.includes('center')) animIn = position.includes('top') ? 'toast-animate-in-top' : 'toast-animate-in-bottom';
        ...
        requestAnimationFrame(() => {
            setTimeout(() => {
                toast.classList.remove('toast-animate-in-top', 'toast-animate-in-bottom', 'toast-animate-in-left', 'toast-animate-in-right', 'opacity-0');
            }, 10);
        });
