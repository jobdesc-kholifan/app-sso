    ...
    menuItems.forEach((item) => {
      ...
      if (!hasChildren) {
        if (
          ...
          link.href = rootPrefix + item.link;
        } else {
          link.href = item.link || "#";
        }
      }
      ...
      const isChildActive =
        hasChildren && item.children.some((child) => checkActive(child));
      ...
      if (hasChildren) {
        ...
        function showSubFloating() {
          ...
          if (!subFloatingMenu) {
            let subSide =
              container.closest(".floating-menu-bg")?.dataset.placementSide ||
              "right";
            ...
            subFloatingMenu.dataset.level =
              parseInt(
                container.closest(".floating-menu-bg")?.dataset.level || 0,
              ) + 1;
            ...
            subFloatingMenu._triggerParentMenu =
              container.closest(".floating-menu-bg");
          }
          ...
          function update() {
            ...
            let sideToken = parentMenu
              ? parentMenu.dataset.placementSide
              : null;
            ...
            computePosition(itemEl, subFloatingMenu, {
              ...
              subFloatingMenu.className = subFloatingMenu.className.replace(
                /\bplacement-\S+/g,
                "",
              );
...
function renderMenu(items, container, depth = 0) {
  ...
  items.forEach((item) => {
    ...
    const isChildActive = hasChildren
      ? item.children.some((child) => checkActive(child))
      : false;
      ...
      function showFloating() {
        ...
        if (!floatingMenu) {
          ...
          floatingMenu.addEventListener("mouseenter", () => {
            if (floatingMenu._clearHideTimeout)
        ...
        function update() {
          ...
          computePosition(link, floatingMenu, {
            ...
            // dynamic arrow placement
            floatingMenu.className = floatingMenu.className.replace(
              /\bplacement-\S+/g,
              "",
            );
...
function renderTopMenu(items, container) {
  ...
  filteredItems.forEach((item) => {
    const estimatedWidth =
      60 +
      item.title.length * 8 +
      (item.children && item.children.length > 0 ? 20 : 0);
  ...
  itemsToRender.forEach((item) => {
    ...
    const isDirectActive = checkActive(item);
    const isChildActive =
      hasChildren && item.children.some((c) => checkBranchActive(c));

    const link = document.createElement(
      hasChildren || item.isMore ? "div" : "a",
    );
    ...
    if (hasChildren || item.isMore) {
      ...
      function showFloating() {
        ...
        function update() {
          const optimalSide = getOptimalSide(link);
          const basePlacement =
            optimalSide === "right" ? "bottom-start" : "bottom-end";
